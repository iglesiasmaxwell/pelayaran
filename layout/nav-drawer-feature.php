...
<nav class="pt-6 px-10 flex items-center justify-between 2xl:justify-around hidden md:flex uk-link-reset select-none">
    ...
    <div class="flex font-lexend uppercase text-[12px] text-dark/80 lg:block hidden">
        <a href="../index.php" class="mx-5 font-medium hover:!text-dark transition ease-in duration-150"><?= $nav_menu_multi_array[0][0] ?></a>
        <a href="features.php?index=0" class="mx-5 font-medium hover:!text-dark transition ease-in duration-150"><?= $nav_menu_multi_array[3][0] ?></a>
    </div>
...
<div id="drawer-navigation" class="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-light w-70 uk-link-reset" tabindex="-1" aria-labelledby="drawer-navigation-label">
    ...
    <div class="py-8 overflow-y-auto">
        <ul class="space-y-2 text-dark">
            ...
            <li>
                <a href="features.php?index=0" class="flex items-center p-2 text-dark rounded-md hover:bg-dark-tp/10 transition duration-100">
            ...
            <ul class="pt-6 space-y-2 text-dark">
                <li>
                    <span class="px-2 uppercase font-medium font-lexend text-dark-tp"><?= $nav_menu_multi_array[3][0] ?></span>
                </li>
                <?php
                for ($i = 0; $i < count($features_multi_array); $i++) {
                ?>
                    <li>
                        <a href="features.php?index=<?= $i ?>" class="flex items-center p-2 text-dark rounded-md hover:bg-dark-tp/10 transition duration-100">
                            <i class="fa-<?= $features_icon_style[$i] ?> fa-<?= $features_array_icon[$i][1] ?> fa-fw fa-sm text-dark"></i>
                            <span class="ml-5 font-montserrat tracking font-semibold"><?= $features_multi_array[$i][0] ?></span>
                        </a>
                    </li>
                <?php
                }
